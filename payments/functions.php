 ...
 * You can not use Payment to create a SAAS or Payment-like business. For more details, visit https://boxcoin.dev/terms-of-service (see 5. Intellectual Property and Content Ownership).
 * © 2022-2023 boxcoin.dev. All rights reserved.
 *
 */

define('BXC_VERSION', '1.2.5');
...
function bxc_transactions_create($amount, $cryptocurrency_code, $currency_code = false, $external_reference = '', $title = '', $description = '', $url = false, $billing = '', $vat = false, $checkout_id = false, $user_details = false, $discount_code = false, $type = 1) {
    ...
    if ($cryptocurrency_code === 'btc_ln' && bxc_settings_get('bxc_checkout_urlln-node-active')) {
...
function bxc_usd_rates($currency_code = false) {
    ...
    if (!$fiat_rates || $fiat_rates[0] < (time() - 3600)) {
        ...
        if (!$app_id) {
            ...
            $error = 'Missing Open Exchange Rates App ID. Set it in the Payment settings area. ';
        }
...
function bxc_login($username, $password) {
    ...
    if (!$is_success_login && defined('BXC_AGENTS')) {
    ...
    if ($is_success_login) {
        ...
        if (bxc_settings_get('notifications-login')) {
            $language = bxc_settings_get('language-admin');
            bxc_email_notification(bxc_m('New login', $language), str_replace(['{T}', '{T2}'], [BXC_URL . 'admin.php', date('Y-m-d H:i:s')], bxc_m('New Payment login at the URL {T}. Date and time of access: {T2}.', $language)));
        }
...
function bxc_error($message, $function_name, $force = false) {
    $message = 'Payment error [' . $function_name . ']: ' . (is_string($message) ? $message : json_encode($message));
...
function bxc_vat_validation($vat_number) {
    ...
    if (!$key) {
        return bxc_error('Missing Vatsense key. Set it in the Payment settings area.', 'bxc_vat_validation', true);
    }
