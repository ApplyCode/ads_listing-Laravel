...
class FrontendController extends Controller
{
    ...
    public function fetchAds(Request $request)
    {
        ...
        try {
            // Handle country
            if ($country = $request->route('country')) {
                $countryRecord = Country::whereSortname($country)->first();
                if ($countryRecord) {
                    session()->put('selected_country_slug', $countryRecord->slug);
                    session()->put('selected_country_name', $countryRecord->name);
                    session()->put('selected_country', $countryRecord->name);
                    session()->forget('no_selected_country');
                } else {
                    session()->forget('selected_country_slug');
                ...
                session()->forget('selected_country_name');
                session()->forget('selected_country');
                session()->put('no_selected_country', true);
            }
            ...
            $query = Ad::query()
                ...
                ->latest('id')
                ->with(['category'])
                ->when($country, function ($q, $country) {
                    $q->where('country', Country::whereSortname($country)->first()?->name);
                })
                ->when($request->route('category'), function ($q, $category) {
                    return $q->whereHas('category', function ($q) use ($category) {
                        $q->where('slug', $category);
                    });
                });
    
            // Fetch and paginate ads
            $pathSegments = $request->segments();
            $data['brands'] = Brand::all();
            $data['headerSearch'] = $request->headerSearch ?? '';
            $data['headerLocation'] = $request->headerLocation ?? '';
            $data['ads'] = $query->paginate(20)->withQueryString();
            $data['metatitle'] = $query->count().' '.__('ads_found_for').'-';
    ...
    public function setSelectedCountry(Request $request)
    {
        try {
            ...
            if (str_contains($previousUrl, 'ads/')) {
                preg_match('/(ads.*)/s', $previousUrl, $matches);
                $newUrl = selected_country() ? selected_country()->sortname . '/' . $matches[1] : $matches[1];
            }
