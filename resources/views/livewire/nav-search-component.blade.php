<div class="flex-grow flex justify-center" x-data="{ 'open': @entangle('location_search_modal').defer }">
    ...
    <div class="w-full max-w-[488px] xl:max-w-[625px]">
            ...
            <form action="{{ route('frontend.ads') }}" method="GET"
                ...
                <label for="location" class="w-full bg-white group-hover:bg-gray-50 cursor-pointer overflow-hidden ltr:rounded-r-lg rtl:rounded-l-lg location">
                    ...
                    <span @click="open = true, $wire.openModal()" x-transection
                        ...
                        text="location">{{ $search ? Str::limit($search, 20) : selected_country()?->name ?? __('location') }}</span>
                ...
                <button type="submit" class="absolute top-0 ltr:right-0 rtl:left-0 py-2.5 bg-gray-700 text-white heading-07 rounded-lg px-4 hover:bg-primary-700">{{ __('search') }}</button>
            </form>
        ...
        <!-- search location -->
        <div id="location-modal" x-show="open" style="display: none">
            <div class="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[60] w-full max-w-2xl max-h-full">
                ...
                <button id="close-btn-modal" @click="open = false"
...
<script>
    window.addEventListener('update-url', ({detail}) => {
        const currentPath = window.location.pathname
        if (currentPath.includes('ads')) {
            let newPath = '/'
            newPath += detail.country ? detail.country + '/' : ''
            newPath += detail.state ? detail.state.replaceAll(' ', '-') + '/' : ''
            newPath += detail.city ? detail.city.replaceAll(' ', '-') + '/' : ''

            history.replaceState(null, '', newPath + getSubString(currentPath, 'ads'))
        }
    })


    const getSubString = (str, startString) => {
        let startIndex = str.indexOf(startString);
